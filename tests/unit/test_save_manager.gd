...
const TEST_TMP_PATH = "user://test_save_data.tmp"
...
func test_atomic_write_uses_tmp() -> void:
	...
	assert_false(FileAccess.file_exists(TEST_TMP_PATH), "TMP file should be gone after save")
	assert_true(FileAccess.file_exists(TEST_SAVE_PATH), "SAVE file should exist")

func test_save_manager_autoload_constants() -> void:
	assert_eq(SaveManager.SAVE_PATH, "user://save.json", "SAVE_PATH constant should be correct")
	assert_eq(SaveManager.TMP_PATH, "user://save.tmp", "TMP_PATH constant should be correct")
